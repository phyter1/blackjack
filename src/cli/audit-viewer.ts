...
class AuditViewer {
  ...
  async start(): Promise<void> {
    ...
    console.log(chalk.green.bold("           🔍 BLACKJACK AUDIT TRAIL VIEWER 🔍           "));
    console.log(chalk.green.bold("═".repeat(60)));
    ...
    while (true) {
      const action = await select({
        ...
        choices: [
          { name: "View Audit Trail (from current session)", value: "view_current" },
  ...
  private displayAllEvents(events: any[]): void {
    console.log(chalk.yellow("\n📋 All Events:\n"));
    for (const event of events.slice(0, 50)) { // Limit to first 50
      console.log(chalk.white(`[${event.timestamp}] ${chalk.cyan(event.type)}`));
  ...
  private displaySummary(events: any[]): void {
    ...
    for (const [type, count] of Object.entries(summary).sort((a, b) => b[1] - a[1])) {
      console.log(chalk.cyan(`${type.padEnd(30)} ${chalk.white(count.toString().padStart(5))}`));
    }
  }
  ...
  private async filterByRound(events: any[]): Promise<void> {
    const rounds = [...new Set(events.map(e => e.roundNumber).filter(r => r !== undefined))];

    const selectedRound = await select({
      message: "Select round number:",
      choices: rounds.map(round => ({ name: `Round ${round}`, value: round })),
    });
