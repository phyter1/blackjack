...
export function CasinoTable({
  ...
  const setShowTrainerSidebar = useUIStore((state) => state.setShowTrainerSidebar);
  const showSettingsDialog = useUIStore((state) => state.showSettingsDialog);
  const setShowSettingsDialog = useUIStore((state) => state.setShowSettingsDialog);
  ...
  const handleInsuranceAction = useGameStore((state) => state.handleInsuranceAction);
  ...
  useEffect(() => {
    if (phase === "settling") {
      const updateSettlementOutcomes = useGameStore.getState().updateSettlementOutcomes;
