...
import type { UserProfile, UserBank, GameSession } from "@/types/user";
...
export function TerminalGamePersistent({
  ...
  const endGame = () => {
    if (!session || !player || !game) return;

    game.endSession();

    // End session in UserService
    ...
    if (summary) {
      ...
      addLine([
        {
          text: `  Session Profit/Loss: ${profit >= 0 ? "+" : ""}$${profit.toFixed(2)}\n`,
