  ...
  return (
    <Card className="bg-gray-900 border-gray-700">
      <CardHeader>
        ...
        <CardDescription>
          Variance:
          <span className={variance >= 0 ? "text-green-500 ml-2 font-semibold" : "text-red-500 ml-2 font-semibold"}>
            {formatCurrency(variance)}
          </span>
          <span className="text-gray-400 ml-2">
            ({variance >= 0 ? "+" : ""}{((variance / Math.abs(totalEV)) * 100).toFixed(1)}% vs EV)
          </span>
        </CardDescription>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={data} margin={{ top: 35, right: 20, left: 10, bottom: 5 }}>
            ...
            <Tooltip
              ...
              formatter={(value: any, name: string) => {
                if (typeof value === "number") {
                  const label = name === "expectedValue" ? "Expected Value (EV)" : "Actual Value (AV)";
            ...
            <Legend
              ...
              formatter={(value) => {
                return value === "expectedValue" ? "Expected Value (EV)" : "Actual Value (AV)";
              }}
            />
            <Line
              type="monotone"
